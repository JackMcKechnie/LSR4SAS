...
class TripletIDDistilDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            print("error")
            return ("error", ["error"] * self.train_group_size, [0.0] * self.train_group_size)
        return (q_text, doc_list, score_list)
        

    # def __getitem__(self, idx):
    #     if idx not in self.qrels.keys():
    #         idx = 0
    #     train_group_size = 2
    #     q_id, q_text = self.queries[idx]
    #     pos_index = self.qrels[q_id][0]
    #     neg_index = self.qrels[q_id][1]
    #     pos_doc = self.doc_dict[pos_index]
    #     neg_doc = self.doc_dict[neg_index]
    #     pos_score = self.ce_score[q_id][pos_index]
    #     neg_score = self.ce_score[q_id][neg_index]
    #     return (q_text, [pos_doc, neg_doc], [pos_score, neg_score])

    # def __getitem__(self, idx):
    #     q_id, q_text = self.queries[idx]
    #     if q_id + "poo" in self.qrels:
    #         doc1_id = random.choice(self.qrels[q_id])
    #     else:
    #         if q_id in self.ce_score:
    #             doc1_id = random.choice(list(self.ce_score[q_id].keys()))
    #         else:
    #             return ("error", ["error", "error"], [0.5, 0.5])
    #     doc_list = [self.doc_dict[doc1_id]]
    #     score_list = [self.ce_score[q_id][doc1_id]]
    #     all_doc_ids = list(self.ce_score[q_id].keys())
    #     if len(all_doc_ids) < self.train_group_size - 1:
