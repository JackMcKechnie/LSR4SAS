...
class DistilKLLoss(Loss):
    ...
    KLLoss(q, p1, p8) = KL(normalize([S(q,p1), S(q,p8)]), normalize([T(q,p1), T(q,p8)])).
    """
    ...
    def forward(self, q_reps, d_reps, labels):
        ...
        d_reps: torch.Tensor
            batch of document vectors (size: batch_size*8 x vocab_size).
            ...
            Documents in even positions (0, 8, 4...) are positive (relevant) documents, documents in odd positions (1, 3, 5...) are negative (non-relvant) documents.
        labels: torch.Tensor
            Teacher's margin between positive and negative documents. labels[i] = teacher(q_reps[i], d_reps[i*8]) - teacher(q_reps[i], d_reps[i*8+1])
        Returns
        -------
        ...
        docs = d_reps.view(batch_size, 8, -1)  # shape: (batch_size, 8, dim)
        p_reps = docs[:, 0, :]  # shape: (batch_size, dim)
        n_reps = docs[:, 1:, :]  # shape: (batch_size, 7, dim)
        # p_reps, n_reps = d_reps.view(batch_size, 8, -1).transpose(0, 1)
        teacher_scores = torch.softmax(labels.view(batch_size, 8), dim=1)
        ...
        # Expand q_reps to (batch_size, 7, dim) to match n_reps
        q_expanded = q_reps.unsqueeze(1).expand_as(n_reps)  # (batch_size, 7, dim)
        n_rel = dot_product(q_expanded, n_reps)  # (batch_size, 7)
        
        # student_scores = torch.stack([p_rel, n_rel], dim=1)
        # student_scores = torch.log_softmax(student_scores, dim=1)
        student_scores = torch.cat([p_rel.unsqueeze(1), n_rel], dim=1)  # (batch_size, 8)
        student_scores = torch.log_softmax(student_scores, dim=1)
        reg_q_output = (
        ...
        reg_d_output = (
            ...
            else (self.d_regularizer(p_reps) + self.d_regularizer(n_reps)) / 8
        )
