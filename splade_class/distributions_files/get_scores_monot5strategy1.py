...
args.strategy = args.strategy.split(".")[0]
...
qrels = read_qrels(f'./{args.strategy}.json')
...
monot5_scorer = MonoT5ReRanker(model = model_path, batch_size = 64, verbose = True)
...
def ordered_dict(group):
    sorted_rows = group.sort_values(by="rank")  # Ensure rank 0 is first
    return {row["docno"]: row["score"] for _, row in sorted_rows.iterrows()}

output_dict = scored.groupby("qid").apply(ordered_dict).to_dict()
