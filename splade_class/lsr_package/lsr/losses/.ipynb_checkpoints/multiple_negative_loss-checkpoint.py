...
class MultipleNegativeLoss(Loss):
    ...
    def __init__(self, q_regularizer=None, d_regularizer=None, sens_reg=None) -> None:
        """
        ...
        super(MultipleNegativeLoss, self).__init__(q_regularizer, d_regularizer, sens_reg)
    ...
    def forward(self, q_reps, d_reps, labels=None):
        ...
        sens_reg_output = (
            torch.tensor(0.0, device=d_reps.device)
            if (self.sens_reg is None)
            else self.sens_reg(d_reps)
        )
        ...
        to_log = {
            ...
            "sensitivity reg" : sens_reg_output,
            "ce loss" : ce_loss.detach(),
            "combined_loss" : ce_loss + sens_reg_output + reg_q_output + reg_d_output
        }
