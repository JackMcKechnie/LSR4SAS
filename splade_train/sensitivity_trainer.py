...
class IRTrainer(BaseTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        docs = model(**inputs) # shape (bsz, 1, Vocab), (bsz, nb_neg+1, Vocab)
        if self.lexical_type != "none":
            input_ids = inputs["input_ids"].reshape(-1,2,inputs['input_ids'].size(1)) #(bsz, nb_neg+2, seq_length)
            ...
            docs = docs * doc_bow.view(-1, 2, doc_bow.size(-1))
        ...
        positive_docs = docs[:, :1, :]  # shape: (bsz, 1, doc_dim)
        
        # Extract the negative documents
        negative_docs = docs[:, 1:, :]  # shape: (bsz, nb_neg, doc_dim)
        
        # Reshape negative docs for batch-wise computation
        negatives = negative_docs.reshape(-1, docs.size(2)).T  # shape: (doc_dim, bsz * nb_neg)
        
        # Compute similarity scores between the positive docs and all negative docs
        scores_positive = torch.matmul(positive_docs.squeeze(1), negatives)  # shape: (bsz, bsz * nb_neg)
        
        # Concatenate the positive document scores with their corresponding negative scores
        all_scores = torch.cat([scores_positive], dim=1)  # shape: (bsz, bsz * nb_neg)
