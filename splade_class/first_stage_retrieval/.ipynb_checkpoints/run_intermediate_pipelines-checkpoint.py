...
pipelines = [
    bm25 % p,
    bm25 % p >> filter % k,
    bm25 % p >> filter % k >> get_text >> monot5_relevance,
    ...
    bm25 % p,
    bm25 % p >> filter % k,
    bm25 % p >> filter % k >> get_text >> monot5_sans,
    
    splade_relevance % p,
    splade_relevance % p >> filter % k,
    splade_relevance % p >> filter % k >> get_text >> monot5_relevance,
    ...
    splade_relevance % p,
    splade_relevance % p >> filter % k,
    splade_relevance % p >> filter % k >> get_text >> monot5_sans,

    splade_sans % p,
    splade_sans % p >> filter % k,
    splade_sans % p >> filter % k >> get_text >> monot5_relevance,
    ...
    splade_sans % p,
    splade_sans % p >> filter % k,
    splade_sans % p >> filter % k >> get_text >> monot5_sans,

    splade_distil % p,
    splade_distil % p >> filter % k,
    splade_distil % p >> filter % k >> get_text >> monot5_relevance,
    ...
    splade_distil % p,
    splade_distil % p >> filter % k,
    splade_distil % p >> filter % k >> get_text >> monot5_sans,
...
names = [
    f"bm25 % {p}",
    f"bm25 % {p} >> filter % {k}",
    f"bm25 % {p} >> filter % {k} >> get_text >> monot5_relevance",
    ...
    f"bm25 % {p}",
    f"bm25 % {p} >> filter % {k}",
    f"bm25 % {p} >> filter % {k} >> get_text >> monot5_sans",
    
    f"splade_relevance % {p}",
    f"splade_relevance % {p} >> filter % {k}",
    f"splade_relevance % {p} >> filter % {k} >> get_text >> monot5_relevance",
    ...
    f"splade_relevance % {p}",
    f"splade_relevance % {p} >> filter % {k}",
    f"splade_relevance % {p} >> filter % {k} >> get_text >> monot5_sans",
    
    f"splade_sans % {p}",
    f"splade_sans % {p} >> filter % {k}",
    f"splade_sans % {p} >> filter % {k} >> get_text >> monot5_relevance",
    ...
    f"splade_sans % {p}",
    f"splade_sans % {p} >> filter % {k}",
    f"splade_sans % {p} >> filter % {k} >> get_text >> monot5_sans",
    
    f"splade_distil % {p}",
    f"splade_distil % {p} >> filter % {k}",
    f"splade_distil % {p} >> filter % {k} >> get_text >> monot5_relevance",
    ...
    f"splade_distil % {p}",
    f"splade_distil % {p} >> filter % {k}",
    f"splade_distil % {p} >> filter % {k} >> get_text >> monot5_sans",
...
for pipeline, name in zip(pipelines, names):
    ...
    pt.io.write_results(results, f"./runs/intermediates/{name}")
...
# exp.to_csv("./runs/results.csv", mode='a', header=not os.path.exists("./runs/results.csv"), index=False)